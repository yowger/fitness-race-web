...
const raceStatusStyles: Record<string, { label: string; className: string }> = {
    upcoming: {
        label: "Upcoming",
...
export default function RunnerProfilePage({ id }: { id: string }) {
    ...
    const { data: userResults } = useRunnerResultsPaginated({
        status: "complete",
        userId: user?.id || "",
    })
                                    ...
                                    {userResults?.results?.map((race) => {
                                        ...
                                        return (
                                            <div key={race.race_id}>
                                                <Link
                                                    to={`/dashboard/races/${race.race_id}/complete`}
                                                >
                                                    <div className="race-card bg-white rounded-lg border-2 border-gray-200 overflow-hidden shadow-sm">
                                                        <div className="flex">
                                                            {/* Banner */}
                                                                ...
                                                                <div className="flex items-start justify-between mb-2">
                                                                    <div>
                                                                        ...
                                                                        <div className="flex items-center gap-3 text-sm text-zinc-500">
                                                                            ...
                                                                            {status && (
                                                                                <span
                                                                                    className={`inline-block mt-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${raceStatusStyles[status]?.className}`}
                                                                                >
                                                                                    {
                                                                                        raceStatusStyles[
                                                                                            status
                        ...
                        {myHostedRaces?.length === 0 && (
                            <div className="text-center py-20 bg-white rounded-lg border-2 border-gray-200 shadow-sm">
                                ...
                                <Link to="/dashboard/races/create">
                                    <button className="cursor-pointer px-8 py-3 bg-gradient-to-r from-cyan-600 to-green-600 text-white font-heading text-lg rounded-lg hover:shadow-xl transition-all inline-flex items-center gap-2">
